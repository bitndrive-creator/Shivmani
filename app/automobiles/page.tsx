...
function getImg(coverImage?: string, category?: string): string {
  ...
  return CAT_IMAGE_MAP[category ?? ''] ?? '/images/automobiles.jpg';
}
...
// ── LAUNCH CARD — No Link, No Enquire ─────────────────────────
function LaunchCard({ car, big = false }: { car: typeof LAUNCHES[0]; big?: boolean }) {
  ...
  return (
    // ✅ div instead of Link — no navigation on click
    <div
      onMouseEnter={() => setHov(true)}
      onMouseLeave={() => setHov(false)}
      style={{ display: 'block', background: '#0F0F0F', border: `1px solid rgba(201,168,76,${hov ? '0.35' : '0.1'})`, transition: 'all .3s', boxShadow: hov ? '0 12px 40px rgba(0,0,0,0.3)' : 'none', cursor: 'default' }}
    >
  ...
  useEffect(() => {
    const fetchPosts = async () => {
      try {
        ...
        const categoryPosts = data.filter(p => p.isPublished && p.category === THIS_CATEGORY);
        setEditorialPosts(categoryPosts);
      } catch (err) {
