...
const CATS: Cat[] = [
  ...
  { name: 'Beauty',              slug: 'beauty',            icon: '💄', description: "India's finest luxury beauty, skincare and wellness",           heroImage: '/images/beauty-elton.jpg'        },
...
function BlogCard({ post, big = false }: { post: Post; big?: boolean }) {
  ...
  return (
    <Link
      ...
      style={{ display: 'flex', flexDirection: 'column', textDecoration: 'none', background: hov ? '#F5F0E8' : '#FAFAF8', transition: 'background .2s', height: '100%', border: '1px solid rgba(0,0,0,0.06)' }}
      onMouseEnter={() => setHov(true)} onMouseLeave={() => setHov(false)}
    >
      <div style={{ position: 'relative', paddingBottom: '62%', overflow: 'hidden', background: '#1A1A1A', flexShrink: 0 }}>
        <img
          src={imgSrc}
          alt={post.title}
        ...
        <span style={{ position: 'absolute', top: 14, left: 14, fontSize: 11, letterSpacing: '0.18em', textTransform: 'uppercase', color: '#C9A84C', background: 'rgba(10,10,10,0.85)', padding: '5px 10px', border: '1px solid rgba(201,168,76,0.25)' }}>
          {post.category}
        </span>
      </div>

      <div style={{ padding: '18px 20px 20px', display: 'flex', flexDirection: 'column', flex: '1 1 auto' }}>
        <div style={{ width: hov ? 40 : 20, height: 1, background: '#C9A84C', marginBottom: 14, transition: 'width .35s ease' }} />
        <h3 style={{ fontFamily: 'Georgia, serif', fontWeight: 400, fontSize: big ? 24 : 20, lineHeight: 1.4, color: hov ? '#8B6914' : '#1A1A1A', marginBottom: 10, transition: 'color .25s', letterSpacing: '0.01em', display: '-webkit-box', WebkitLineClamp: big ? 2 : 3, WebkitBoxOrient: 'vertical', overflow: 'hidden' }}>
          {post.title}
        </h3>
        ...
        <div style={{ marginTop: 'auto', display: 'flex', alignItems: 'center', flexWrap: 'nowrap', gap: 8, borderTop: '1px solid rgba(201,168,76,0.1)', paddingTop: 12, overflow: 'hidden' }}>
          <span style={{ fontSize: 12, letterSpacing: '0.1em', color: 'rgba(107,101,88,0.5)', whiteSpace: 'nowrap' }}>By</span>
          <span style={{ fontSize: 12, letterSpacing: '0.15em', textTransform: 'uppercase', color: '#C9A84C', fontWeight: 500, whiteSpace: 'nowrap', overflow: 'hidden', textOverflow: 'ellipsis', flex: '1 1 auto', minWidth: 0 }}>{post.author}</span>
          <span style={{ fontSize: 12, color: 'rgba(107,101,88,0.45)', letterSpacing: '0.04em', whiteSpace: 'nowrap', flex: '0 0 auto' }}>{formatDate(post.createdAt)} · {post.readTime} min</span>
        </div>
      </div>
    </Link>
  );
  ...
  return (
    ...
    >
      <div style={{ position: 'relative', paddingBottom: '70%', background: '#1A1A1A', overflow: 'hidden' }}>
        ...
        <div style={{ position: 'absolute', inset: 0, display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'flex-end', padding: '0 16px 24px', textAlign: 'center' }}>
          <span style={{ fontSize: 28, marginBottom: 8, filter: 'drop-shadow(0 2px 8px rgba(0,0,0,0.6))', transform: hov ? 'translateY(-6px)' : 'translateY(0)', transition: 'transform .35s' }}>{cat.icon}</span>
          <h3 style={{ fontFamily: 'Georgia, serif', fontWeight: 400, fontSize: 16, color: '#fff', letterSpacing: '0.03em', lineHeight: 1.2, textShadow: '0 2px 12px rgba(0,0,0,0.7)', transform: hov ? 'translateY(-4px)' : 'translateY(0)', transition: 'transform .35s', margin: 0 }}>{cat.name}</h3>
...
export default function HomePage() {
  ...
  const heroPost = posts.length === 0 ? null : [...posts].sort((a, b) => (a.homepage?.heroPriority ?? 999) - (b.homepage?.heroPriority ?? 999))[0];
  ...
  const middlePosts    = remainingPosts.slice(2, 5);
  const bottomPosts    = remainingPosts.slice(5, 8);
    ...
    <>
      ...
      <style>{`
        ...
        @media(max-width:767px){
          .ilh-hero        { height: 100svh !important; min-height: 580px !important; }
          .ilh-hero-inner  { padding: 0 16px 72px !important; }
          .ilh-section-pad { padding: 56px 16px !important; }
          ...
          .ilh-partners-grid { grid-template-columns: repeat(3,1fr) !important; }
          .ilh-heading-row { flex-direction: column !important; align-items: flex-start !important; gap: 12px !important; }
        }

        /* ── Tablet ── */
        @media(min-width:768px) and (max-width:1023px){
          .ilh-section-pad { padding-left: 24px !important; padding-right: 24px !important; }
                    .ilh-grid-5      { grid-template-columns: repeat(2,1fr) !important; }
      ...
      <main style={{ overflowX: 'hidden', width: '100%' }}>
        ...
        {/* ══ SECTION 1: HERO ══════════════════════════════════ */}
        <section style={{ position: 'relative', background: '#0A0A0A', minHeight: '100svh', display: 'flex', flexDirection: 'column' }}>

          {/* Image — covers full section height */}
          {prevIdx !== null && (
            <img src={HERO_IMAGES[prevIdx]} alt="ILH Hero"
              style={{ position: 'absolute', inset: 0, width: '100%', height: '100%', objectFit: 'cover', objectPosition: 'center center', opacity: fading ? 0 : 1, transition: 'opacity 1.2s ease-in-out', zIndex: 1 }} />
          )}
          <img key={heroIdx} src={HERO_IMAGES[heroIdx]} alt="ILH Hero"
            style={{ position: 'absolute', inset: 0, width: '100%', height: '100%', objectFit: 'cover', objectPosition: 'center center', opacity: fading ? 0 : 1, transition: 'opacity 1.2s ease-in-out', zIndex: 2 }} />

          {/* Overlay */}
          <div style={{ position: 'absolute', inset: 0, zIndex: 3, background: 'linear-gradient(to top, rgba(10,10,10,0.92) 0%, rgba(10,10,10,0.4) 50%, rgba(10,10,10,0.15) 100%)' }} />

          {/* Content — centered */}
          <div style={{ position: 'relative', zIndex: 10, flex: 1, display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center', textAlign: 'center', padding: '100px 24px 80px' }}>
            <p style={{ fontSize: 13, letterSpacing: '0.45em', textTransform: 'uppercase', color: '#FFFFFF', textShadow: '0 2px 16px rgba(0,0,0,0.9)', fontWeight: 600, marginBottom: 16, animation: 'hFadeUp .8s .1s ease both' }}>
              Where India Meets Global Luxury
            </p>
            <h1 style={{ fontFamily: 'Georgia, serif', fontSize: 'clamp(32px, 7vw, 88px)', fontWeight: 300, color: '#FAFAF8', lineHeight: 1.08, marginBottom: 20, animation: 'hFadeUp .8s .2s ease both', letterSpacing: '0.015em' }}>
              Discover the Finest<br /><em style={{ color: '#DFC27A', fontStyle: 'italic' }}>in Indian Luxury</em>
            </h1>
            <p style={{ fontSize: 16, color: 'rgba(250,250,248,0.92)', fontWeight: 400, letterSpacing: '0.03em', marginBottom: 36, animation: 'hFadeUp .8s .3s ease both', maxWidth: 520, textShadow: '0 2px 12px rgba(0,0,0,0.9)' }}>
              Real estate, automobiles, jewellery, weddings, and curated luxury experiences — through an India-first lens.
            </p>
            <div style={{ display: 'flex', gap: 14, flexWrap: 'wrap', justifyContent: 'center', animation: 'hFadeUp .8s .4s ease both' }}>
              <a href="#categories"
                style={{ fontSize: 10, letterSpacing: '0.25em', textTransform: 'uppercase', color: '#1A1A1A', background: '#C9A84C', padding: '13px 28px', textDecoration: 'none', fontWeight: 600, transition: 'all .3s' }}
                onMouseEnter={e => { e.currentTarget.style.background = '#DFC27A'; }}
                onMouseLeave={e => { e.currentTarget.style.background = '#C9A84C'; }}
              >Explore Categories</a>
              <Link href="/partner-with-us"
                style={{ fontSize: 10, letterSpacing: '0.25em', textTransform: 'uppercase', color: '#C9A84C', border: '1px solid rgba(201,168,76,0.5)', padding: '13px 28px', textDecoration: 'none', transition: 'all .3s' }}
        ...
        <section id="categories" className="ilh-section-pad" style={{ background: '#0A0A0A', padding: '80px 32px', borderBottom: '1px solid rgba(201,168,76,0.1)' }}>
          <div style={{ maxWidth: 1280, margin: '0 auto' }}>
            <div style={{ textAlign: 'center', marginBottom: 40 }}>
              <p style={{ fontSize: 18, letterSpacing: '0.4em', textTransform: 'uppercase', color: 'rgba(201,168,76,0.6)', marginBottom: 12 }}>Explore</p>
              <h2 style={{ fontFamily: 'Georgia, serif', fontSize: 'clamp(26px, 4vw, 48px)', fontWeight: 300, color: '#FAFAF8', letterSpacing: '0.02em' }}>Our Categories</h2>
              <div style={{ width: 60, height: 1, background: 'linear-gradient(90deg, transparent, #C9A84C, transparent)', margin: '18px auto 0' }} />
            </div>
            <div style={{ display: 'grid', gridTemplateColumns: 'repeat(6, 1fr)', gap: 10 }} className="ilh-grid-5">
        ...
        <section className="ilh-section-pad" style={{ background: '#FAFAF8', padding: '80px 32px' }}>
          <div style={{ maxWidth: 1280, margin: '0 auto' }}>
            <div className="ilh-heading-row" style={{ display: 'flex', alignItems: 'flex-end', justifyContent: 'space-between', marginBottom: 36, borderBottom: '1px solid rgba(201,168,76,0.15)', paddingBottom: 20 }}>
              <div>
                <p style={{ fontSize: 18, letterSpacing: '0.35em', textTransform: 'uppercase', color: '#DFC27A', fontWeight: 600, marginBottom: 8 }}>Latest from the World of Luxury</p>
                <h2 style={{ fontFamily: 'Georgia, serif', fontSize: 'clamp(24px, 4vw, 44px)', fontWeight: 300, color: '#1A1A1A', letterSpacing: '0.01em' }}>Trending Stories</h2>
              </div>
              <Link href="/news" style={{ fontSize: 11, letterSpacing: '0.2em', textTransform: 'uppercase', color: '#C9A84C', textDecoration: 'none', borderBottom: '1px solid rgba(201,168,76,0.4)', paddingBottom: 2, flexShrink: 0 }}>
                All News →
              </Link>
            </div>
            ...
            {loading && !error && (
              <>
                <div style={{ display: 'grid', gridTemplateColumns: 'repeat(3, 1fr)', gap: 16, marginBottom: 16 }} className="ilh-grid-3">
                  <BlogSkeleton /><BlogSkeleton /><BlogSkeleton />
                </div>
                <div style={{ display: 'grid', gridTemplateColumns: 'repeat(3, 1fr)', gap: 16, marginBottom: 16 }} className="ilh-grid-3">
                  <BlogSkeleton /><BlogSkeleton /><BlogSkeleton />
                </div>
                <div style={{ display: 'grid', gridTemplateColumns: 'repeat(3, 1fr)', gap: 16 }} className="ilh-grid-3">
                  <BlogSkeleton /><BlogSkeleton /><BlogSkeleton />
                </div>
              </>
            )}
            ...
            {!loading && !error && (
              sortedPosts.length === 0 ? (
                ...
                <>
                  <div style={{ display: 'grid', gridTemplateColumns: 'repeat(3, 1fr)', gap: 16, marginBottom: 16, alignItems: 'stretch' }} className="ilh-grid-3">
                  ...
                  {bottomPosts.length > 0 && (
                    <div style={{ display: 'grid', gridTemplateColumns: 'repeat(3, 1fr)', gap: 16, alignItems: 'stretch' }} className="ilh-grid-3">
        ...
        {/* ══ SECTION 4: PARTNERS ══════════════════════════════ */}
        <section className="ilh-section-pad" style={{ background: '#0A0A0A', padding: '72px 32px', borderTop: '1px solid rgba(201,168,76,0.1)', borderBottom: '1px solid rgba(201,168,76,0.1)' }}>
          <div style={{ maxWidth: 1280, margin: '0 auto', textAlign: 'center' }}>
            <p style={{ fontSize: 18, letterSpacing: '0.4em', textTransform: 'uppercase', color: 'rgba(201,168,76,0.5)', marginBottom: 12 }}>Our Network</p>
            <h2 style={{ fontFamily: 'Georgia, serif', fontSize: 'clamp(22px, 3vw, 36px)', fontWeight: 300, color: '#FAFAF8', marginBottom: 40, letterSpacing: '0.02em' }}>
              Trusted by India&apos;s Premium Businesses
            </h2>
            <div style={{ display: 'grid', gridTemplateColumns: 'repeat(6, 1fr)', gap: 1, border: '1px solid rgba(201,168,76,0.1)' }} className="ilh-partners-grid ilh-grid-4">
              {PARTNERS.map((p, i) => (
                <div key={p.name}
                  style={{ padding: '28px 16px', display: 'flex', alignItems: 'center', justifyContent: 'center', borderRight: i < 5 ? '1px solid rgba(201,168,76,0.08)' : 'none', transition: 'background .25s', cursor: 'pointer' }}
                  onMouseEnter={e => (e.currentTarget.style.background = 'rgba(201,168,76,0.05)')}
                  onMouseLeave={e => (e.currentTarget.style.background = 'transparent')}
                >
                  <div style={{ textAlign: 'center' }}>
                    <div style={{ width: 52, height: 52, background: 'rgba(201,168,76,0.1)', borderRadius: '50%', margin: '0 auto 10px', display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
                      <span style={{ fontSize: 20, color: '#C9A84C' }}>◈</span>
                    </div>
                    <p style={{ fontSize: 11, letterSpacing: '0.12em', textTransform: 'uppercase', color: 'rgba(201,168,76,0.55)', margin: 0 }}>{p.name}</p>
                  </div>
                </div>
              ))}
            </div>
            <Link href="/curated-partners"
              style={{ display: 'inline-block', marginTop: 36, fontSize: 11, letterSpacing: '0.25em', textTransform: 'uppercase', color: 'rgba(201,168,76,0.6)', textDecoration: 'none', borderBottom: '1px solid rgba(201,168,76,0.3)', paddingBottom: 2, transition: 'color .2s' }}
        ...
        {/* ══ SECTION 5: WHY ILH ═══════════════════════════════ */}
        <section className="ilh-section-pad" style={{ background: '#FAFAF8', padding: '88px 32px' }}>
          <div style={{ maxWidth: 1280, margin: '0 auto' }}>
            <div style={{ textAlign: 'center', marginBottom: 52 }}>
              <p style={{ fontSize: 18, letterSpacing: '0.4em', textTransform: 'uppercase', color: '#C9A84C', marginBottom: 12 }}>Our Promise</p>
              <h2 style={{ fontFamily: 'Georgia, serif', fontSize: 'clamp(24px, 4vw, 44px)', fontWeight: 300, color: '#1A1A1A', letterSpacing: '0.01em' }}>Why Indian Luxury House</h2>
              <div style={{ width: 60, height: 1, background: 'linear-gradient(90deg, transparent, #C9A84C, transparent)', margin: '18px auto 0' }} />
            </div>
            <div style={{ display: 'grid', gridTemplateColumns: 'repeat(4, 1fr)', gap: 2 }} className="ilh-grid-why">
              {WHY_ILH.map((item, i) => (
                <div key={i}
                  style={{ padding: '36px 28px', background: '#fff', border: '1px solid rgba(201,168,76,0.1)', transition: 'all .3s', cursor: 'default' }}
                ...
                >
                  <div style={{ fontSize: 26, color: '#C9A84C', marginBottom: 18, opacity: 0.8 }}>{item.icon}</div>
                  <h3 style={{ fontFamily: 'Georgia, serif', fontSize: 20, fontWeight: 400, color: 'inherit', marginBottom: 12, letterSpacing: '0.01em', lineHeight: 1.35 }}>{item.title}</h3>
                  <p style={{ fontSize: 14, color: 'rgba(107,101,88,0.75)', lineHeight: 1.8, fontWeight: 300 }}>{item.desc}</p>
                </div>
              ))}
            </div>
          </div>
        ...
        {/* ══ SECTION 6: CTA ═══════════════════════════════════ */}
        <section className="ilh-section-pad" style={{ background: '#0A0A0A', padding: '100px 32px', position: 'relative', overflow: 'hidden' }}>
          ...
          <div style={{ maxWidth: 800, margin: '0 auto', textAlign: 'center', position: 'relative', zIndex: 1 }}>
            <p style={{ fontSize: 18, letterSpacing: '0.45em', textTransform: 'uppercase', color: 'rgba(201,168,76,0.55)', marginBottom: 18 }}>Grow With Us</p>
            <h2 style={{ fontFamily: 'Georgia, serif', fontSize: 'clamp(26px, 5vw, 60px)', fontWeight: 300, color: '#FAFAF8', lineHeight: 1.15, marginBottom: 22, letterSpacing: '0.01em' }}>
              Build Your Presence in<br /><em style={{ color: '#DFC27A', fontStyle: 'italic' }}>India&apos;s Luxury Market</em>
            </h2>
            <p style={{ fontSize: 16, color: 'rgba(250,250,248,0.4)', fontWeight: 300, lineHeight: 1.8, maxWidth: 520, margin: '0 auto 44px' }}>
            ...
            <Link href="/partner-with-us"
              style={{ display: 'inline-block', fontSize: 11, letterSpacing: '0.3em', textTransform: 'uppercase', color: '#1A1A1A', background: '#C9A84C', padding: '15px 44px', textDecoration: 'none', fontWeight: 700, transition: 'all .3s' }}
              onMouseEnter={e => { e.currentTarget.style.background = '#DFC27A'; }}
              onMouseLeave={e => { e.currentTarget.style.background = '#C9A84C'; }}
            >Partner With Us</Link>
          </div>
