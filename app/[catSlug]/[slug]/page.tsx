...
function getGalleryImg(url: string): string {
  if (!url) return '';
  ...
  return (
    <header style={{
      ...
      <div style={{ maxWidth: 1400, margin: '0 auto', padding: '0 32px', height: 72, display: 'flex', alignItems: 'center', justifyContent: 'space-between', gap: 16 }}>
        ...
        {/* ── LOGO using logos.png ── */}
        <Link href="/" style={{ display: 'flex', alignItems: 'center', gap: 10, textDecoration: 'none', flexShrink: 0 }}>
          <div style={{ position: 'relative', width: 44, height: 44, flexShrink: 0 }}>
            <Image src="/logos.png" alt="Indian Luxury House" fill style={{ objectFit: 'contain' }} priority />
          </div>
          <div style={{ display: 'flex', flexDirection: 'column', justifyContent: 'center' }}>
            <span style={{ fontFamily: 'Georgia,serif', fontSize: 18, fontWeight: 300, color: '#DFC27A', letterSpacing: '0.28em', textTransform: 'uppercase', lineHeight: 1.1 }}>Indian</span>
            <span style={{ fontSize: 10, color: '#C9A84C', letterSpacing: '0.28em', textTransform: 'uppercase', marginTop: 3, fontFamily: 'Georgia,serif' }}>Luxury House</span>
          </div>
        </Link>
...
function Lightbox({ images, startIndex, onClose }: { images: GalleryItem[]; startIndex: number; onClose: () => void }) {
  ...
  return (
    <div onClick={onClose} style={{ position: 'fixed', inset: 0, zIndex: 1000, background: 'rgba(5,5,5,0.96)', display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
      <button onClick={onClose} style={{ position: 'absolute', top: 24, right: 28, background: 'none', border: 'none', color: 'rgba(201,168,76,0.7)', fontSize: 28, cursor: 'pointer', lineHeight: 1, zIndex: 10 }}>✕</button>
      {images.length > 1 && (
        <button onClick={e => { e.stopPropagation(); setCurrent(c => (c - 1 + images.length) % images.length); }}
          style={{ position: 'absolute', left: 20, background: 'none', border: '1px solid rgba(201,168,76,0.25)', color: '#C9A84C', fontSize: 22, cursor: 'pointer', padding: '10px 16px', zIndex: 10 }}>‹</button>
      )}
      <div onClick={e => e.stopPropagation()} style={{ maxWidth: '88vw', maxHeight: '88vh', position: 'relative' }}>
        {/* eslint-disable-next-line @next/next/no-img-element */}
        <img src={getGalleryImg(images[current].url)} alt={images[current].caption || `Gallery ${current + 1}`}
          style={{ maxWidth: '88vw', maxHeight: '82vh', objectFit: 'contain', display: 'block' }} />
        {images[current].caption && (
          <p style={{ textAlign: 'center', fontSize: 11, color: 'rgba(201,168,76,0.55)', marginTop: 12, letterSpacing: '0.06em' }}>{images[current].caption}</p>
        )}
        <p style={{ textAlign: 'center', fontSize: 9, color: 'rgba(255,255,255,0.2)', marginTop: 8, letterSpacing: '0.14em' }}>{current + 1} / {images.length}</p>
      </div>
      {images.length > 1 && (
        <button onClick={e => { e.stopPropagation(); setCurrent(c => (c + 1) % images.length); }}
          style={{ position: 'absolute', right: 20, background: 'none', border: '1px solid rgba(201,168,76,0.25)', color: '#C9A84C', fontSize: 22, cursor: 'pointer', padding: '10px 16px', zIndex: 10 }}>›</button>
      )}
    </div>
  );
}
...
export default function BlogDetailPage({ params }: { params: Promise<{ catSlug: string; slug: string }> }) {
    ...
    <>
      ...
      <div style={{ paddingTop: 72, minHeight: '80vh', background: '#FAFAF8', display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
        <div style={{ textAlign: 'center', padding: '60px 24px' }}>
          ...
          <p style={{ fontSize: 13, color: 'rgba(107,101,88,0.55)', marginBottom: 36 }}>{error ?? 'This article may have been moved or removed.'}</p>
          <Link href="/" style={{ fontSize: 10, letterSpacing: '0.25em', textTransform: 'uppercase', color: '#1A1A1A', background: '#C9A84C', padding: '14px 36px', textDecoration: 'none', fontWeight: 600 }}>Back to Home</Link>
        </div>
      </div>
    </>
  );
  ...
  const galleryImages = (post.gallery ?? []).filter(g => g.type === 'image' && g.url);
  ...
  return (
    <>
      ...
      {lightboxOpen && galleryImages.length > 0 && (
        <Lightbox images={galleryImages} startIndex={lightboxIndex} onClose={() => setLightboxOpen(false)} />
      )}
      ...
      <main style={{ paddingTop: 72 }}>
        ...
        <section className="ilh-hero-section" style={{ position: 'relative', height: 540, overflow: 'hidden', background: '#0A0A0A' }}>
          ...
          <div className="ilh-hero-inner" style={{ position: 'relative', zIndex: 10, height: '100%', display: 'flex', flexDirection: 'column', justifyContent: 'flex-end', maxWidth: 1280, margin: '0 auto', padding: '0 48px 52px' }}>
            ...
            <div style={{ display: 'flex', flexWrap: 'wrap', alignItems: 'center', gap: 16, fontSize: 15, letterSpacing: '0.16em', textTransform: 'uppercase', color: 'rgba(201,168,76,0.5)' }}>
            ...
            <div>
              ...
              {galleryImages.length > 0 && (
                <div style={{ marginTop: 52, marginBottom: 8 }}>
                  <div style={{ display: 'flex', alignItems: 'center', gap: 14, marginBottom: 20, paddingBottom: 14, borderBottom: '1px solid rgba(201,168,76,0.12)' }}>
                    <p style={{ fontSize: 9, letterSpacing: '0.3em', textTransform: 'uppercase', color: 'rgba(201,168,76,0.55)', margin: 0 }}>Gallery</p>
                    <span style={{ fontSize: 9, color: 'rgba(201,168,76,0.3)', letterSpacing: '0.1em' }}>{galleryImages.length} {galleryImages.length === 1 ? 'Photo' : 'Photos'}</span>
                  </div>
                  <div style={{ display: 'grid', gridTemplateColumns: galleryImages.length === 1 ? '1fr' : galleryImages.length === 2 ? '1fr 1fr' : galleryImages.length === 3 ? '1.4fr 1fr 1fr' : 'repeat(2, 1fr)', gap: 6 }}>
                    {galleryImages.map((item, idx) => {
                      const src = getGalleryImg(item.url);
                      const isBig = galleryImages.length >= 3 && idx === 0;
                      return (
                        <div key={item._id} onClick={() => { setLightboxIndex(idx); setLightboxOpen(true); }}
                          style={{ position: 'relative', paddingBottom: isBig ? '80%' : '66%', overflow: 'hidden', background: '#1A1A1A', cursor: 'pointer', ...(galleryImages.length > 3 && idx === 0 ? { gridRow: 'span 2', paddingBottom: '100%' } : {}) }}
                          className="gallery-thumb"
                        >
                          {/* eslint-disable-next-line @next/next/no-img-element */}
                          <img src={src} alt={item.caption || `Gallery image ${idx + 1}`}
                            style={{ position: 'absolute', inset: 0, width: '100%', height: '100%', objectFit: 'cover', transition: 'transform .5s ease' }}
                            className="gallery-img"
                            onError={e => { (e.target as HTMLImageElement).style.display = 'none'; }}
                          />
                          <div className="gallery-overlay" style={{ position: 'absolute', inset: 0, background: 'rgba(10,10,10,0)', transition: 'background .3s', display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
                            <svg className="gallery-icon" width="28" height="28" viewBox="0 0 24 24" fill="none" stroke="#C9A84C" strokeWidth="1.5" style={{ opacity: 0, transition: 'opacity .3s' }}>
                              <circle cx="11" cy="11" r="8"/><line x1="21" y1="21" x2="16.65" y2="16.65"/>
                              <line x1="11" y1="8" x2="11" y2="14"/><line x1="8" y1="11" x2="14" y2="11"/>
                            </svg>
                          </div>
                          {item.caption && (
                            <p style={{ position: 'absolute', bottom: 0, left: 0, right: 0, background: 'linear-gradient(transparent, rgba(5,5,5,0.75))', color: 'rgba(201,168,76,0.8)', fontSize: 10, padding: '20px 12px 10px', letterSpacing: '0.04em', margin: 0 }}>{item.caption}</p>
                          )}
                        </div>
                      );
                    })}
                  </div>
                  <p style={{ fontSize: 9, color: 'rgba(107,101,88,0.35)', marginTop: 10, letterSpacing: '0.08em' }}>Click any image to view full screen · Use arrow keys to navigate</p>
                </div>
              )}
              ...
              {post.tags && post.tags.length > 0 && (
                <div style={{ marginTop: 52, paddingTop: 32, borderTop: '1px solid rgba(201,168,76,0.12)', display: 'flex', flexWrap: 'wrap', gap: 8, alignItems: 'center' }}>
                  <span style={{ fontSize: 15, letterSpacing: '0.22em', textTransform: 'uppercase', color: 'rgba(107,101,88,0.45)', marginRight: 6 }}>Tags:</span>
                  {post.tags.map(tag => (
                    <span key={tag} style={{ fontSize: 9, letterSpacing: '0.14em', textTransform: 'uppercase', color: '#6B6558', background: '#F0EBE0', border: '1px solid rgba(201,168,76,0.15)', padding: '5px 12px' }}>{tag}</span>
                  ))}
                </div>
              )}
              ...
              <div style={{ marginTop: 52, padding: '26px 30px', background: '#F5F0E8', borderLeft: '3px solid #C9A84C', display: 'flex', alignItems: 'flex-start', gap: 20 }}>
                <div style={{ width: 52, height: 52, borderRadius: '50%', background: '#0A0A0A', display: 'flex', alignItems: 'center', justifyContent: 'center', flexShrink: 0 }}>
                  <span style={{ fontFamily: 'Georgia,serif', fontSize: 20, color: '#C9A84C', fontWeight: 300 }}>
                    {post.author.split(' ').map(n => n[0]).join('').slice(0, 2)}
                  </span>
                </div>
                <div>
                  <p style={{ fontSize: 15, letterSpacing: '0.22em', textTransform: 'uppercase', color: 'rgba(201,168,76,0.5)', marginBottom: 6 }}>Written by</p>
                  ...
                  <p style={{ fontSize: 14, color: '#6B6558', lineHeight: 1.7, letterSpacing: '0.01em' }}>
              ...
              <div style={{ position: 'sticky', top: 92, display: 'flex', flexDirection: 'column', gap: 24 }}>
                ...
                {moreRelated.length > 0 && (
                  <div style={{ border: '1px solid rgba(201,168,76,0.14)', padding: '20px' }}>
                    <p style={{ fontSize: 15, letterSpacing: '0.26em', textTransform: 'uppercase', color: '#C9A84C', marginBottom: 14, paddingBottom: 12, borderBottom: '1px solid rgba(201,168,76,0.1)' }}>
                    ...
                    {moreRelated.map((p, i) => {
                      ...
                      return (
                        <Link key={p._id} href={`/${pSlug}/${p.slug}`}
                          style={{ display: 'flex', gap: 12, paddingBottom: i < moreRelated.length - 1 ? 14 : 0, marginBottom: i < moreRelated.length - 1 ? 14 : 0, borderBottom: i < moreRelated.length - 1 ? '1px solid rgba(201,168,76,0.08)' : 'none', textDecoration: 'none' }}
                        >
                          ...
                          <div>
                            <p style={{ fontSize: 12, letterSpacing: '0.16em', textTransform: 'uppercase', color: '#C9A84C', marginBottom: 5 }}>{p.category}</p>
                ...
                {/* ✅ FIX: Sidebar logo using logos.png instead of SVG */}
                <div style={{ background: '#0A0A0A', padding: '26px 20px', textAlign: 'center', border: '1px solid rgba(201,168,76,0.15)' }}>
                  <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'center', gap: 10, marginBottom: 14 }}>
                    <div style={{ position: 'relative', width: 40, height: 40, flexShrink: 0 }}>
                      <Image src="/logos.png" alt="Indian Luxury House" fill style={{ objectFit: 'contain' }} />
                    </div>
                    <div style={{ textAlign: 'left' }}>
                      <div style={{ fontFamily: 'Georgia,serif', fontSize: 14, fontWeight: 300, color: '#DFC27A', letterSpacing: '0.2em', textTransform: 'uppercase', lineHeight: 1.1 }}>Indian</div>
                      <div style={{ fontSize: 8, color: '#C9A84C', letterSpacing: '0.22em', textTransform: 'uppercase', marginTop: 2 }}>Luxury House</div>
                    </div>
                  </div>
                  <p style={{ fontFamily: 'Georgia,serif', fontSize: 20, fontWeight: 300, color: '#FAFAF8', margin: '0 0 6px', letterSpacing: '0.02em' }}>India&apos;s Luxury Circle</p>
                  <p style={{ fontSize: 14, color: 'rgba(201,168,76,0.45)', marginBottom: 18, lineHeight: 1.65, letterSpacing: '0.02em' }}>Curated luxury intelligence, every week.</p>
                ...
                <div style={{ border: '1px solid rgba(201,168,76,0.2)', padding: '20px', background: '#F5F0E8' }}>
                  <p style={{ fontSize: 15, letterSpacing: '0.25em', textTransform: 'uppercase', color: '#C9A84C', marginBottom: 10 }}>For Businesses</p>
                  ...
                  <p style={{ fontSize: 13, color: '#6B6558', lineHeight: 1.65, marginBottom: 16 }}>Feature your brand in front of India&apos;s most affluent readers.</p>
                ...
                <div style={{ border: '1px solid rgba(201,168,76,0.14)', padding: '16px 20px' }}>
                  <p style={{ fontSize: 15, letterSpacing: '0.26em', textTransform: 'uppercase', color: 'rgba(201,168,76,0.45)', marginBottom: 12 }}>Share This Story</p>
...
// // ✅ Image URL resolver — localhost + relative + absolute sab handle
// function getImg(coverImage?: string, category?: string): string {
//   if (coverImage) {
//     if (coverImage.includes('localhost')) {
//       const filename = coverImage.split('/uploads/')[1];
...
// // ✅ NEW: Lightbox for gallery fullscreen view
// function Lightbox({ images, startIndex, onClose }: { images: GalleryItem[]; startIndex: number; onClose: () => void }) {
//   const [current, setCurrent] = useState(startIndex);
...
//           style={{ position: 'absolute', left: 20, background: 'none', border: '1px solid rgba(201,168,76,0.25)', color: '#C9A84C', fontSize: 22, cursor: 'pointer', padding: '10px 16px', zIndex: 10 }}
//         >‹</button>
//       )}
...
//         {/* eslint-disable-next-line @next/next/no-img-element */}
//         <img
//           src={getGalleryImg(images[current].url)}
//           alt={images[current].caption || `Gallery ${current + 1}`}
//           style={{ maxWidth: '88vw', maxHeight: '82vh', objectFit: 'contain', display: 'block' }}
//         />
//         {images[current].caption && (
//           <p style={{ textAlign: 'center', fontSize: 11, color: 'rgba(201,168,76,0.55)', marginTop: 12, letterSpacing: '0.06em' }}>
//             {images[current].caption}
//           </p>
//         )}
//         <p style={{ textAlign: 'center', fontSize: 9, color: 'rgba(255,255,255,0.2)', marginTop: 8, letterSpacing: '0.14em' }}>
...
//           style={{ position: 'absolute', right: 20, background: 'none', border: '1px solid rgba(201,168,76,0.25)', color: '#C9A84C', fontSize: 22, cursor: 'pointer', padding: '10px 16px', zIndex: 10 }}
//         >›</button>
//       )}
//     </div>
//   );
...
//                   {/* Header */}
//                   <div style={{ display: 'flex', alignItems: 'center', gap: 14, marginBottom: 20, paddingBottom: 14, borderBottom: '1px solid rgba(201,168,76,0.12)' }}>
//                     <p style={{ fontSize: 9, letterSpacing: '0.3em', textTransform: 'uppercase', color: 'rgba(201,168,76,0.55)', margin: 0 }}>
//                       Gallery
//                     </p>
//                     <span style={{ fontSize: 9, color: 'rgba(201,168,76,0.3)', letterSpacing: '0.1em' }}>
...
//                     {galleryImages.map((item, idx) => {
//                       const src = getGalleryImg(item.url);
//                       // First image taller agar 3+ hain
//                       const isBig = galleryImages.length >= 3 && idx === 0;
//                       return (
//                         <div
//                           key={item._id}
...
//                           style={{
//                             position: 'relative',
//                             paddingBottom: isBig ? '80%' : '66%',
//                             overflow: 'hidden',
//                             background: '#1A1A1A',
//                             cursor: 'pointer',
//                             // 4 se zyada images mein first item double height
//                             ...(galleryImages.length > 3 && idx === 0 ? { gridRow: 'span 2', paddingBottom: '100%' } : {}),
//                           }}
//                           className="gallery-thumb"
//                         >
//                           {/* eslint-disable-next-line @next/next/no-img-element */}
...
//                               background: 'linear-gradient(transparent, rgba(5,5,5,0.75))',
//                               color: 'rgba(201,168,76,0.8)',
//                               fontSize: 10, padding: '20px 12px 10px',
...
//                     })}
//                   </div>
//                   <p style={{ fontSize: 9, color: 'rgba(107,101,88,0.35)', marginTop: 10, letterSpacing: '0.08em' }}>
...
//   },
//   {
//     id: '8', slug: 'luxury-bridal-looks-2026', category: 'Curated Partners',
...
//   // slug se dhundo — ID se nahi (duplicates avoid karne ke liye)
//   const post = ALL_POSTS.find(p => p.slug === slug) ?? ALL_POSTS[0];
//   const catMeta = CATEGORIES.find(c => c.name === post.category);
//   // slug-based exclude — ID duplicate hone par bhi safe rahega
//   const related = ALL_POSTS.filter(p => p.slug !== post.slug && p.category === post.category).slice(0, 3);
//   const moreRelated = related.length < 3
//     ? [...related, ...ALL_POSTS.filter(p => p.slug !== post.slug && p.category !== post.category).slice(0, 3 - related.length)]
//     : related;
...
//     title: 'Branded Residences Rise: How Global Luxury Hotels Are Redefining Indian Real Estate',
//     excerpt: 'From Four Seasons to Ritz-Carlton, global hospitality giants are reimagining luxury living in India\'s most coveted addresses.',
//     imageUrl: '/images/placeholder-home.jpg',
//     author: 'Priya Mehta', publishedAt: 'April 28, 2026', readTime: 8, featured: true,
//     tags: ['Branded Residences', 'Luxury Real Estate', 'India', 'Investment'],
//     content: `<p>A decade ago, the concept of a branded residence — a private home managed by a world-class hotel group — barely registered in India's luxury property market. Today, it is the most coveted address category in the country, with waiting lists that stretch years and premiums that can reach 40% over comparable unbranded properties.</p>
...
//             <SMLogo size={32} />
//             <span style={{ fontFamily: 'Georgia,serif', fontSize: 16, fontWeight: 300, color: '#DFC27A', letterSpacing: '0.2em' }}>Indian Luxury House</span>
//           </div>
...
//             ))}
//           </div>
//           <p style={{ fontSize: 10, letterSpacing: '0.12em', color: 'rgba(201,168,76,0.24)' }}>© 2026 Indian Luxury House. All rights reserved.</p>
//         </div>
//       </footer>
